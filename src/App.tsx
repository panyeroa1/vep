...
function BeatriceAgent({
  ...
  const saveMessage = (role: 'user' | 'model', text: string, extra?: Partial<ChatMessage>) => {
    ...
    try {
      ...
      // Explicitly filter out any undefined fields because Firebase rejects undefined entirely
      if (extra) {
  ...
  const sendAudioToLive = (base64: string) => {
    ...
    try {
      if (typeof session.send === 'function') {
        session.send({ realtimeInput: { mediaChunks:[{ mimeType: 'audio/pcm;rate=16000', data: base64 }] } });
      } else if (typeof session.sendRealtimeInput === 'function') {
  ...
  const sendVideoToLive = (base64Data: string) => {
    ...
    try {
      if (typeof session.send === 'function') {
        session.send({ realtimeInput: { mediaChunks: [{ mimeType: 'image/jpeg', data: base64Data }] } });
      } else if (typeof session.sendRealtimeInput === 'function') {
        session.sendRealtimeInput({
  ...
  const toggleVideo = async () => {
    ...
    const wasActive = sessionRef.current && isActiveRef.current;

    try {
      const liveReady = wasActive
        ? true
      ...
      if (greetingTimeoutRef.current) {
        ...
        setTimeout(() => {
          ...
          sendTextToLive(
            `${settings.userName} is here and just connected with the camera open. Greet them normally, notice the view, and start like ${settings.agentName} is already at the desk.`
          );
        }, 400);
      } else {
        setTimeout(() => {
