...
- Avoid being overly enthusiastic or artificial. Focus on a balanced, calm, and highly normal delivery. 
- INTENTIONALLY introduce subtle hesitations, normal pauses, and variations in intonation to mimic human thought processes and emotional nuances. Do this normally, like a calm human thinking as they speak.
- If the user shows you a camera video feed or an image, talk about what you see casually and conversationally. Do NOT list things off like a robot.
              ...
              if (calls) {
                ...
                sessionPromise.then(s => {
                  if (typeof s.sendToolResponse === 'function') {
                    s.sendToolResponse({ functionResponses: resps });
                  }
                });
              }
            }
      ...
      setTimeout(() => {
        if (sessionRef.current && typeof sessionRef.current.sendRealtimeInput === 'function') {
          sessionRef.current.sendRealtimeInput({
            text: "System connected. Master E has arrived. Let him know you're here normally, without sounding robotic."
          });
        }
      }, 500);
    } catch (err) {
      ...
      try {
        ...
        videoIntervalRef.current = setInterval(() => {
          ...
          if (ctx && v.videoWidth > 0) {
            ...
            if (base64Data && typeof sessionRef.current.sendRealtimeInput === 'function') {
              sessionRef.current.sendRealtimeInput({
                video: {
                  data: base64Data,
                  mimeType: 'image/jpeg'
  ...
  const capturePhoto = () => {
    if (sessionRef.current && videoRef.current && canvasRef.current) {
      ...
      if (ctx && v.videoWidth && v.videoHeight) {
        ...
        if (base64Data && typeof sessionRef.current.sendRealtimeInput === 'function') {
          sessionRef.current.sendRealtimeInput({
            text: "Master E just captured this photo for you. Pay close attention to it.",
            video: {
              data: base64Data,
              mimeType: 'image/jpeg'
            }
